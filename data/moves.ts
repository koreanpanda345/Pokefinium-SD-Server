...
export const Moves: {[moveid: string]: MoveData} = {
	...
	round: {
		...
		roundhouse: {
				...
				basePower: 75,
				basePowerCallback(pokemon, target, move) {
					if(this.lastMove?.volatileStatus !== undefined) {
						if(target.moveLastTurnResult === false && this.lastMove.volatileStatus[0] === "flinch")
						{
							return move.basePower * 2;
						}
					}
	...
	skillswap: {
		...
		onHit(target, source, move) {
			...
			if (targetAbility.id !== sourceAbility.id) {
				...
				target.abilityData = {id: toID(target.ability), target};
				if (target.side !== source.side) target.staleness = 'external';
			}
